...
const mapDoctor = (d) => ({
    ...
    hospitalLogoUrl: (0, imageUrl_1.getImageUrl)(d.hospital_logo, 'businesses'),
});
// ── Hub ──────────────────────────────────────────────────────────────────────
router.get('/', async (_req, res, next) => {
    try {
        const [specialties, featured, emirates] = await Promise.all([
            (0, pool_1.query)(`SELECT bc.id, bc.name, bc.icon,
            ...
            (0, pool_1.query)(`${DOCTOR_SELECT} WHERE d.is_active=1 AND b.is_active=1 ORDER BY d.is_featured DESC, d.rating DESC LIMIT 10`),
            (0, pool_1.query)(`SELECT DISTINCT b.emirate FROM doctors d JOIN businesses b ON b.id=d.business_id WHERE b.emirate IS NOT NULL ORDER BY b.emirate`),
        ]);
        const total = await (0, pool_1.queryOne)('SELECT COUNT(*) AS n FROM doctors WHERE is_active=1');
...
router.get('/list', async (req, res, next) => {
    try {
        const { specialty, hospital, emirate, search, available_today, top_rated, sort, page = '1', pageSize = '30' } = req.query;
        ...
        if (hospital) {
            where.push('d.business_id=?');
            params.push(hospital);
        }
        if (emirate) {
            where.push('b.emirate=?');
            params.push(emirate);
        }
        if (available_today === '1')
            where.push("d.availability LIKE 'Available Today%'");
        if (top_rated === '1')
            where.push('d.rating >= 4.5');
        if (search) {
            where.push('(d.name LIKE ? OR sc.name LIKE ? OR b.name LIKE ?)');
            params.push(`%${search}%`, `%${search}%`, `%${search}%`);
        }
        const whereSql = where.join(' AND ');
        let order = 'd.is_featured DESC, d.rating DESC';
        if (sort === 'fee_low')
            order = 'd.consultation_fee ASC';
        else if (sort === 'fee_high')
            order = 'd.consultation_fee DESC';
        else if (sort === 'rating' || top_rated === '1')
            order = 'd.rating DESC, d.review_count DESC';
        else if (sort === 'experience')
            order = 'd.experience_years DESC';
        const offset = (Number(page) - 1) * Number(pageSize);
        const totalRow = await (0, pool_1.queryOne)(`SELECT COUNT(*) AS total FROM doctors d JOIN businesses b ON b.id=d.business_id LEFT JOIN business_categories sc ON sc.id=d.specialty_id WHERE ${whereSql}`, params);
        const items = await (0, pool_1.query)(`${DOCTOR_SELECT} WHERE ${whereSql} ORDER BY ${order} LIMIT ? OFFSET ?`, [...params, Number(pageSize), offset]);
        let specName = 'Doctors';
        if (specialty) {
            const s = await (0, pool_1.queryOne)('SELECT name FROM business_categories WHERE id=?', [specialty]);
            if (s)
                specName = s.name;
        }
...
router.get('/:id', async (req, res, next) => {
    try {
        const d = await (0, pool_1.queryOne)(`${DOCTOR_SELECT} WHERE d.id=? AND d.is_active=1`, [Number(req.params.id)]);
        if (!d)
